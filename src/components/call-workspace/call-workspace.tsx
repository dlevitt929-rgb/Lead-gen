...
import { Phone, PhoneOff, CheckCircle2, Clock, XCircle, Loader2, MapPin, Star, ExternalLink, X, MessageCircle } from "lucide-react";
...
export function CallWorkspace({ queue, onExit, onQueueChange }: { queue: OpportunityRow[]; onExit: () => void; onQueueChange: (q: OpportunityRow[]) => void }) {
  ...
  async function logOutcome(outcome: CallOutcome, dueAt?: string) {
    if (!lead) return;

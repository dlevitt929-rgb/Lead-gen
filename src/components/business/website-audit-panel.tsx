...
import { Gauge, Loader2, AlertCircle, RefreshCw, Globe } from "lucide-react";
...
export function WebsiteAuditPanel({
  ...
  if (!website) {
    return (
      <EmptyState
        icon={Globe}
        title="No website to audit"
        description="This business doesn't have a public website — that's the strongest possible opportunity signal. Lead with that on the call."
      />
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-xs text-muted-foreground">
          Audited {formatDateTime(latestAudit.performedAt)} via{" "}
          {latestAudit.source === "pagespeed" ? "Google PageSpeed Insights" : "heuristic scan"}
        </div>
